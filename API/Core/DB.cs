...
namespace API.Core
{
    public class DB : IDisposable
    {
        ...
        public void CreateDatabase()
        {
            ...
            NewCommand(@"SELECT COUNT(*) FROM User WHERE role = 'admin';");
            var result = Execute();

            if (result is SqliteDataReader reader && reader.Read() && reader.GetInt32(0) == 0)
            {
                NewCommand(@"INSERT INTO User (email, name, password, role) 
                     VALUES ('admin@example.com', 'Admin', 'admin123', 'admin');");
                Execute();
            }
